...
const signupMiddleware = async (req, res, next) => {
    ...
    if(password !== confirmation )return res.status(422).send("Password fields must have the same values");
...
const loginMiddleware = async (req, res, next) => {
    ...
    try {
        ...
        if(!realUser) return res.status(422).send("This email is not registered");
    } catch (error) {
        console.error(error);
        return res.status(500).send("Internal issue, please try again later");
    }
